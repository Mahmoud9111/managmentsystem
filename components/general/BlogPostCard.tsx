...
export function BlogPostCard({data}:IappProps) {
    return(
        <>
            <div className="group relative overflow-hidden rounded-lg border-gary-200 bg-white shadow-md transition-all hover:shadow-lg">
                <Link href={`/post/${data.id}`} className="block w-full h-full">
                    <div className="relative h-48 w-ful overflow-hidden">
                        <Image src={data.imageUrl} alt="image for blog" fill className="object-cover transition-transform duration-300 group-hover:scale-110"/>
                    </div>
                    <div className="p-4">
